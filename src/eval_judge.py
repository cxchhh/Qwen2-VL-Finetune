...
def bot_streaming(images, task, subtask, processor:AutoProcessor,  generation_args):

    prompt = f"<image>\n<image>\nBased on the third-person view and the first-person view, is the subtask '{subtask}' in robotic task '{task}' finished or still ongoing?"
...
def main(args):
    ...
    tot = acc = o_f = f_o = 0
    for data in tqdm.tqdm(judge_data):
        ...
        judgement = bot_streaming_with_args(images, task, subtask)

        print(task,",", subtask,",", judgement, res)

        if judgement == "ongoing" and res == "finished":
            o_f += 1
        elif judgement == "finished" and res == "ongoing":
            f_o += 1
        else:
            acc += 1
        ...
        if tot % 100 == 0:
            ...
            o_f_rate = o_f / tot * 100
            f_o_rate = f_o / tot * 100
            print(
                f"[{tot}] ✅ {acc:6d} ({acc_rate:5.2f}%) | ❌ finish→ongoing: {o_f:6d} ({o_f_rate:5.2f}%) | ❌ ongoing→finish: {f_o:6d} ({f_o_rate:5.2f}%)\n",
            )
            # ✅ TensorBoard logging
            writer.add_scalar("Accuracy/Correct", acc_rate, tot)
            writer.add_scalar("Error/Ongoing→Finish", f_o_rate, tot)
            writer.add_scalar("Error/Finish→Ongoing", o_f_rate, tot)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--eval_dataset", type=str, default="/mnt/afs/chenxuchuan/datasets/qwen_ft/judge_val.json")
    ...
    if args.debug:
        ...
        args.eval_dataset = "/mnt/afs/chenxuchuan/datasets/qwen_ft/debug/judge_val.json"
    main(args)
